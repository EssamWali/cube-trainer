...
[![CI](https://github.com/EssamWali/cube-trainer/actions/workflows/ci.yml/badge.svg)](https://github.com/EssamWali/cube-trainer/actions/workflows/ci.yml)
![Python](https://img.shields.io/badge/python-3.10%20%7C%203.11%20%7C%203.12%20%7C%203.13-blue)
![Tests](https://img.shields.io/badge/tests-1420-brightgreen)
...
![The PLL case picker: chosen cases ringed in green, the cursor ringed inside its tile](docs/images/pll-picker.png)
...
| what you are shown | what it was |
| --- | --- |
| ![A hidden case](docs/images/case-t-perm-hidden.png) | ![The same case revealed as a T perm](docs/images/case-t-perm.png) |
 Press `p` to reveal it and its algorithm — that rep is
...
| orientation case | permutation case | F2L case |
| --- | --- | --- |
| ![An OLL case in two tones, no arrows](docs/images/case-oll.png) | ![A PLL case in true colours with arrows](docs/images/case-t-perm.png) | ![An F2L case seen from a corner, last layer greyed](docs/images/case-f2l.png) |
| two tones, no arrows | true colours, arrows | corner view, last layer greyed |
 A cube with a slot still open is an F2L case, drawn as a
...
1,420 tests, about twenty seconds. The move engine is cross-checked against
`kociemba`, an unrelated solver, and the case data is checked against the cube
group rather than against its own algorithms: the suite enumerates all 21
last-layer permutation classes and all 57 orientation classes and requires the
shipped cases to cover each exactly once.

The pictures in this README are generated by the application's own drawing code:

```
python tools/make_docs_images.py
```

CI runs that generator on every push, so a change to the drawing code cannot
quietly break the tool that produces them. It does not compare the images byte
for byte - two machines rasterise text differently and that difference would
mean nothing.
